...
export default function PaperAIExpandPage() {
  ...
  const expandContentSync = trpc.paperAi.expandContent.useMutation({
    onSuccess: (data) => {
  ...
  const expandJobsQuery = trpc.paperAi.listExpandJobs.useQuery(
    { paperId, limit: 10 },
    {
  ...
  const handleExpand = async () => {
    ...
    try {
      ...
      console.error("Async paper expand failed, fallback to sync expand", error);
